...
/** سند توريد — evidences money handed over to the owner out of their property's collections. */
export function RemittanceDocument({ doc, org }: { doc: Doc; org: Org }) {
  const { remittance } = doc;
  const { owner, building } = remittance;
      ...
      <Card className="print:border-0 print:shadow-none">
        <CardContent className="space-y-6 p-6 sm:p-8">
          ...
          <div className="text-center">
            <h1 className="text-xl font-bold">سند توريد للمالك</h1>
            <p className="text-xs text-muted-foreground">Owner Remittance Voucher</p>
          </div>
          ...
          <p className="text-sm leading-7">
            تم توريد مبلغ وقدره <span className="font-bold">{formatCurrency(doc.amount)}</span> (
            {amountToArabicWords(doc.amount)} لا غير) إلى المالك{" "}
            <span className="font-bold">{owner.name}</span>، وذلك من مستحقاته عن عقار{" "}
          ...
          <div>
            ...
            <Row label="طريقة التحويل" value={remittance.method} />
            <Row label="رقم العملية / الشيك" value={remittance.reference} />
          </div>

          {remittance.notes && (
            <div className="rounded-lg border p-3 text-sm">
              <p className="text-xs text-muted-foreground">ملاحظات</p>
              <p className="mt-1 whitespace-pre-wrap">{remittance.notes}</p>
            </div>
          )}

          <div className="grid gap-8 pt-8 sm:grid-cols-2">
            <div className="space-y-8 text-center">
              <p className="text-sm font-bold">المستلم (المالك)</p>
              <p className="text-sm">{owner.name}</p>
              <p className="border-t pt-2 text-xs text-muted-foreground">التوقيع</p>
            </div>
            <div className="space-y-8 text-center">
              <p className="text-sm font-bold">مدير الأملاك</p>
